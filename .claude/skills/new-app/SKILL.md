...
1. **`apps.yaml`** — add the entry (name, repo URL, trunk, production branch) in the strict
   format. Ask the owner for the branch model; default `main` / `deploy/prod`.
2. **The repo itself** — if it doesn't exist yet, offer `gh repo create <org>/<name> --private`
   ...
   possible). Create the production branch from trunk so deploys have a target.
